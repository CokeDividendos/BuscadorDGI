...
def page_analysis() -> None:
    ...
    with right:
        ...
        try:
            ...
            with top_cols[0]:
                _kpi_card(f"### Beta", _fmt_kpi(stats.get("beta")))
            with top_cols[1]:
                pe = stats.get(f"### pe_ttm")
                pe_txt = (_fmt_kpi(pe) + "x") if isinstance(pe, (int, float)) else "N/D"
                _kpi_card(f"### PER (TTM)", pe_txt)
            with top_cols[2]:
                _kpi_card(f"### EPS (TTM)", _fmt_kpi(stats.get("eps_ttm")))
            with top_cols[3]:
                _kpi_card(f"### Target 1Y", _fmt_kpi(stats.get("target_1y")))
